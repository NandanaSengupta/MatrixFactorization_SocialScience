...
#cd("/Users/Nandana/Desktop/GitHub folder/MissingValues/Apr 2017/consistency/NLSY/Data/")
...
@everywhere function rank_bootstrapcv(df, s, k, datatype)
...
@everywhere function reg_search(df, s, datatype)
	...
	#ranks = collect(5:1:7)
	lowtrace1 = @time pmap(k -> rank_bootstrapcv(df,s, k, datatype),ranks)
...
@everywhere function rank_bootstrapfit(df, s, datatype, kfit)
...
@everywhere function mult_imp(df, m, catcol, ordcol)
	...
	for i = 1:m
		...
		glrm_fin = rank_bootstrapfit(df, s, datatype, kfit)
...
############### SAVE RESULTS ############# 
writetable("Rank_NLSY10_MI.csv", imps)
writedlm("Rank_s10.csv", s)
writedlm("Regs10.csv", regs)
