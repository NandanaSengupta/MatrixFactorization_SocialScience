...
@everywhere function trace_bootstrapcv(df, k, datatype)
...
@everywhere function reg_search(df, s, datatype)
	...
	lowtrace1 = @time pmap(k -> trace_bootstrapcv(df, k, datatype),ranks)
...
############### SAVE RESULTS ############# 
writetable("Rank_NLSY20_MI.csv", imps)
writedlm("Rank_s20.csv", s)
writedlm("Regs20.csv", regs)
